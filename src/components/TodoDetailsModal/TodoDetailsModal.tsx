...
import type { Todo, Subtask, TodoUpdates, Priority } from '../../types';
...
interface TodoDetailsModalProps {
  ...
  onClose: () => void;
  todo?: Todo;
  onUpdateTodo: (id: string, updates: TodoUpdates) => void;
  onAddSubtask: (todoId: string, title: string) => void;
  onToggleSubtask: (id: string, completed: boolean) => void;
  onDeleteSubtask: (id: string) => void;
  onUpdateSubtaskPosition?: (id: string, position: number) => void;
}
...
export default function TodoDetailsModal({
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      setLocalSubtasks(newSubtasks);
        ...
        const getPos = (item: Subtask | undefined) => item?.position || 0;
        ...
        let newPosition: number;
         ...
         if (onUpdateSubtaskPosition) {
             onUpdateSubtaskPosition(active.id as string, newPosition);
         }
    }
  };
  ...
  const handleAddSubtaskSubmit = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (newSubtask.trim()) {
