...
describe('useTodos', () => {
    ...
    beforeEach(() => {
        ...
        mockSupabaseChain.then.mockImplementation((resolve: (value: { data: unknown[]; error: null }) => void) => resolve({ data: [], error: null }));
    });
    ...
    it('should fetch and return todos', async () => {
        ...
        mockSupabaseChain.then.mockImplementation((resolve: (value: { data: typeof mockTodos; error: null }) => void) => resolve({ data: mockTodos, error: null }));
    ...
    it('should add a todo', async () => {
        const newTodo = { title: 'New Todo', description: 'desc', priority: 'Low' as const };
        const returnedTodo = { ...newTodo, id: '123', created_at: '2023-01-03T00:00:00Z', tags: [] };

        // Mock fetch return (empty initially)
        mockSupabaseChain.then.mockImplementationOnce((resolve: (value: { data: unknown[]; error: null }) => void) => resolve({ data: [], error: null }));
        ...
        // Mock insert return
        mockSupabaseChain.then.mockImplementation((resolve: (value: { data: typeof returnedTodo[]; error: null }) => void) => resolve({ data: [returnedTodo], error: null }));
    ...
    it('should toggle a todo', async () => {
         // Mock fetch return
         mockSupabaseChain.then.mockImplementationOnce((resolve: (value: { data: { id: string; completed: boolean }[]; error: null }) => void) => resolve({ data: [{id: '1', completed: false}], error: null }));
    ...
    it('should delete a todo', async () => {
         // Mock fetch return
         mockSupabaseChain.then.mockImplementationOnce((resolve: (value: { data: { id: string }[]; error: null }) => void) => resolve({ data: [{id: '1'}], error: null }));
